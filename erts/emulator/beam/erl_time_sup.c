...
static void do_erts_deliver_time(Sint64 current_ms)
{
    ...
    if (((current_ms - erts_smp_atomic_read_nob(&last_delivered_ms)) / CLOCK_RESOLUTION) > 0) {
	erts_time_t elapsed;
#if !defined(USE_LOCKED_GTOD)
    	erts_smp_mtx_lock(&erts_timeofday_mtx);
#endif
	/* calculate and deliver appropriate number of ticks */
	elapsed = (current_ms - erts_smp_atomic_read_nob(&last_delivered_ms)) / CLOCK_RESOLUTION;
	...
	if (elapsed > 0) {
	    ...
	    erts_smp_atomic_set_nob(&last_delivered_ms,current_ms);
	}
...
erts_init_time_sup(void)
{
    ...
    erts_smp_atomic_init_nob(&last_emu_time_us,0);
    ...
    erts_smp_atomic_init_nob(&gtv_ms,init_ms);
...
wall_clock_elapsed_time_both(UWord *ms_total, UWord *ms_diff)
{
    ...
    *ms_total = cur_ms - init_ms;
    prev_total = erts_smp_atomic_xchg_nob(&gtv_ms,cur_ms) - init_ms;
...
get_now(Uint* megasec, Uint* sec, Uint* microsec)
{
    ...
    do {
        then = erts_smp_atomic_read_mb(&then_us);
        if (then >= now_us) {
            now_us = then+1;
        }
...
erts_get_emu_time(SysTimeval *this_emu_time_p)
{
    ...
    do {
        last = erts_smp_atomic_read_mb(&last_emu_time_us);
        if (last >= cur_us) {
            cur_us = last+1;
        }
    } while (erts_smp_atomic_cmpxchg_mb(&last_emu_time_us,cur_us,last) != last);
